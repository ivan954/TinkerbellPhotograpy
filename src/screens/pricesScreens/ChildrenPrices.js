...
const ChildrenPrices = () => {
  ...
  const childrenRef = ref(
    storage,
    "gs://tinkerbellphotograpy.appspot.com/Gallery/Children/Prices/"
  );

  useEffect(() => {
    try {
      listAll(childrenRef).then((response) => {
  ...
  return (
    <Container style={{ marginTop: "10%" }} fluid>
      {loading ? (
        ...
        <Container>
          <Container>
            <h1 className="text-center">צילומי ילדים</h1>
          </Container>
          <Container>
